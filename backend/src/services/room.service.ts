...
export class RoomService {
    ...
    if (checkIn && checkOut) {
      const roomsWithAvailability = await Promise.all(
        rooms.map(async (room) => {
          ...
          return { ...room, available_rooms: room.totalRooms - booked };
        })
      );
      return roomsWithAvailability.filter((r) => r.status === 'available');
    }

    return rooms.map((r) => ({ ...r, available_rooms: r.totalRooms }));
  }
  ...
  async getById(id: number) {
    ...
    return { ...rest, hotel_name: hotel.name, city: hotel.city, address: hotel.address };
  }
